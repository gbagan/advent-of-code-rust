        ...
        for len in start_token.len()..=end_token.len() {
            match len {
                2 => {
                    ...
                    let end = end.min(99);
                    p1 += sum_invalid_patterns(start, end, 11);
                },
                3 => {
                    let start = start.max(100);
                    let end = end.min(999);
                    p2 += sum_invalid_patterns(start, end, 111);
                }
                4 => {
                    let start = start.max(1_000);
                    let end = end.min(9_999);
                    p1 += sum_invalid_patterns(start, end, 101);
                },
                5 => {
                    let start = start.max(10_000);
                    let end = end.min(99_999);
                    p2 += sum_invalid_patterns(start, end, 11_111);
                }
                6 => {
                    let start = start.max(100_000);
                    let end = end.min(999_999);
                    p1 += sum_invalid_patterns(start, end, 1001);
                    p2 += sum_invalid_patterns(start, end, 10_101)
                        - sum_invalid_patterns(start, end, 111_111);
                },
                7 => {
                    let start = start.max(1_000_000);
                    let end = end.min(9_999_999);
                    p2 += sum_invalid_patterns(start, end, 1_111_111);
                },
                8 => {
                    let start = start.max(10_000_000);
                    let end = end.min(99_999_999);
                    p1 += sum_invalid_patterns(start, end, 10_001);
                },
                9 => {
                    let start = start.max(100_000_000);
                    let end = end.min(999_999_999);
                    p2 += sum_invalid_patterns(start, end, 1_001_001);
                },
                10 => {
                    let start = start.max(1_000_000_000);
                    let end = end.min(9_999_999_999);
                    p1 += sum_invalid_patterns(start, end, 100_001);
                    p2 += sum_invalid_patterns(start, end, 101_010_101)
                        - sum_invalid_patterns(start, end, 1_111_111_111);
                },
                _ => {}
            }
        }
...
fn sum_invalid_patterns(start: u64, end: u64, mult: u64) -> u64 {
